...
router.get("/", auth, async (req, res) => {
  try {
    ...
    const result = schedules.map((s) => ({
      ...
      myProgress: (enrolMap[s.id] || []).filter((e) => e.status === "approved")
        .length,
...
router.post("/:id/join", auth, async (req, res) => {
  try {
    ...
    if (!wallet || wallet.balance < schedule.pointsReq) {
      return res
        .status(400)
        .json({ error: "Insufficient Marketing Points to join this campaign" });
    }
    ...
    try {
      created = await prisma.$transaction(async (tx) => {
        ...
        if (alreadyJoined) {
          throw Object.assign(new Error("ALREADY_JOINED"), {
            code: "ALREADY_JOINED",
          });
        }
        const slots = [];
      ...
      if (txErr.code === "ALREADY_JOINED") {
        return res
          .status(400)
          .json({ error: "You have already joined this campaign" });
      }
      throw txErr;
    }
...
router.post("/enrollment/:id/approve", adminAuth, async (req, res) => {
  try {
    ...
    const totalRequired =
      configuredRequired > 0 ? configuredRequired : allSlots.length;
    const totalApproved = allSlots.filter(
      (e) => e.status === "approved",
    ).length;
    ...
    if (totalApproved < totalRequired) {
      ...
      return res.json({
        success: true,
        enrollment: withProofNumber(enrollment),
        rewarded: false,
      });
    }
